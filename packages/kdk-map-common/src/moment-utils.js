...
// regarding currently selected forecast model if any - each meteo_model source is scoped to a specific
// model (eg. a short-lived high-resolution model vs. a longer-horizon global one), so the bound cannot be
// resolved without knowing which model is currently selected. Returns null when unbounded or when the
// layer declares no meteo_model sources for that model at all.
